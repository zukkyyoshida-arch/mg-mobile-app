// ワンタップ追加ボトムシート（モーダル）
// 元 CashLedger.jsx の {showAddModal && (...)} ブロックをそのまま切り出したプレゼンテーショナル
// コンポーネント。フォーム系 state は form、setter は set にまとめて受け取り、
// 冒頭で分解して以降の JSX は元コードと同一に保つ（挙動を1ミリも変えない）。
//
...
function AddTransactionModal({
  ...
  const {
    ...
    setRiskQty,
    setRiskPrice,
    setRiskMonopolyAdQtys,
    setMarketQuantities,
    setSalesData,
    setMachineQuantities,
    setMachineSaleQuantities,
    setAdQuantities,
    setRdPrice,
    setGreenChips,
    setShowCalculator
          ...
          <div className="modal-content" ref={modalContentRef} onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleAddTransaction} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div>
                ...
                {/* 🌟 ルールA (よく使う項目) */}
                <div style={{ marginBottom: '20px', padding: '12px', borderRadius: '12px', background: 'rgba(0, 0, 0, 0.03)', border: '1px solid rgba(0, 0, 0, 0.05)' }}>
                ...
                {/* 📌 ルールB・その他 (決算・随時項目) */}
                <div style={{ padding: '12px', borderRadius: '12px', background: 'rgba(0, 0, 0, 0.04)', border: '1px solid rgba(0, 0, 0, 0.02)' }}>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginBottom: '12px' }}>
                    {[
                      ...
                      <button
                        type="button"
                        key={btn.action}
                        onClick={btn.onClick ? btn.onClick : () => handleCategorySelect(btn.symbol)}
                        className={`btn-premium ${selectedCategory === btn.symbol ? 'btn-primary' : 'btn-secondary'}`}
...
{(selectedCategory === 'オ' || selectedCategory === 'MAX_Loan') && (
  <div style={{fontSize:'0.75rem', color:'var(--text-secondary)', marginTop:'4px'}}>
    最大借入残高の上限: {(currentPeriod <= 3 ? 2 : 3) * ((carryover?.capital || 300) + (carryover?.retainedEarnings || 0))} 万
              ...
              {/* カスタムUIブロック */}
              {selectedCategory !== '期首処理' && (
              <div className="glass-card" style={{ margin: '4px 0', padding: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(0, 0, 0, 0.02)' }}>
              ...
              {selectedCategory === '生産' ? (
                ...
                <div style={{ background: 'rgba(33, 150, 243, 0.05)', padding: '16px', borderRadius: '12px', border: '1px dashed var(--mg-blue)' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                              ...
                              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                ...
                                {maxLimit !== undefined && (
                                  <button
                                    ...
                                    style={{ background: 'var(--color-accent)', border: 'none', color: 'var(--text-primary)', padding: '4px 8px', borderRadius: '4px', fontSize: '0.75rem', fontWeight: 'bold', opacity: (qty >= maxLimit) || totalQty >= maxTotalPurchase ? 0.3 : 1 }}
                          ...
                          <div key={chip.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(0,0,0,0.04)', padding: '8px 12px', borderRadius: '8px', opacity: isBought ? 0.6 : 1 }}>
                            <div>
                              <div style={{ fontWeight: 'bold', fontSize: '0.9rem', display: 'flex', alignItems: 'center', gap: '6px' }}>
                                {chip.name}
                                {isBought && <span style={{ fontSize: '0.65rem', padding: '2px 6px', background: 'rgba(0,0,0,0.1)', borderRadius: '4px' }}>購入済</span>}
                              </div>
                ...
                (() => {
                  const held = {
                    large: results?.largeMachines || 0,
                    small: results?.smallMachines || 0,
                    attachment: results?.attachments || 0
                  };
                  const totalSold = (machineSaleQuantities.large || 0) + (machineSaleQuantities.small || 0) + (machineSaleQuantities.attachment || 0);
                  ...
                  return (
                    <div style={{ background: 'rgba(233, 30, 99, 0.1)', padding: '16px', borderRadius: '12px', border: '1px solid rgba(233, 30, 99, 0.3)' }}>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '8px', marginBottom: '16px' }}>
                        {MACHINES.map(m => {
                          const qty = machineSaleQuantities[m.id] || 0;
                          const maxHeld = held[m.id] || 0;

                          return (
                            <div key={m.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(0,0,0,0.02)', padding: '8px 12px', borderRadius: '8px' }}>
                              <div style={{ display: 'flex', flexDirection: 'column' }}>
                                <span style={{ fontWeight: '700', fontSize: '0.9rem' }}>{m.name}</span>
                                <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>保有: {maxHeld}台</span>
                              </div>
                              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                <button
                                  type="button"
                                  onClick={() => setMachineSaleQuantities(prev => ({ ...prev, [m.id]: Math.max(0, qty - 1) }))}
                                  style={{ background: 'rgba(0,0,0,0.05)', border: 'none', color: 'var(--text-primary)', width: '28px', height: '28px', borderRadius: '4px', fontSize: '1.2rem', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                                >-</button>
                                <span style={{ width: '20px', textAlign: 'center', fontWeight: 'bold', fontSize: '1rem' }}>{qty}</span>
                                <button
                                  type="button"
                                  onClick={() => setMachineSaleQuantities(prev => ({ ...prev, [m.id]: Math.min(maxHeld, qty + 1) }))}
                                  disabled={qty >= maxHeld}
                                  style={{ background: 'rgba(233, 30, 99, 0.3)', border: 'none', color: 'var(--text-primary)', width: '28px', height: '28px', borderRadius: '4px', fontSize: '1.2rem', display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: qty >= maxHeld ? 0.3 : 1 }}
                                >+</button>
                              </div>
                            </div>
                          );
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px', background: 'rgba(0,0,0,0.04)', borderRadius: '8px' }}>
                        <span style={{ color: 'var(--text-secondary)', fontSize: '0.85rem' }}>売却台数: <strong style={{ color: 'var(--text-primary)', fontSize: '1rem' }}>{totalSold}台</strong></span>
                        <span style={{ color: 'var(--text-secondary)', fontSize: '0.85rem' }}>入金額: <strong style={{ color: 'var(--mg-pink)', fontSize: '1.1rem' }}>{Number(amount) || 0}万</strong></span>
                      </div>
                    </div>
                  );
                })()
